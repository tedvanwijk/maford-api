...
function convertDbNameToParamName(input) {
    ...
    let indexOccurence = newInput.indexOf('_', 0);
    while (indexOccurence >= 0) {
        underscoreIndices.push(indexOccurence);
        indexOccurence = newInput.indexOf('_', indexOccurence + 1);
    }
    underscoreIndices = [...underscoreIndices, -1];
    for (let i = 0; i < underscoreIndices.length; i++) {
        newInput = newInput.substring(0, underscoreIndices[i] + 1) + newInput.charAt(underscoreIndices[i] + 1).toUpperCase() + newInput.substring(underscoreIndices[i] + 2)
    }
    newInput = newInput.replaceAll('_', '');
    return newInput
}

async function getAdditionalSpecificationParameters(data) {
    console.log(data)
    const toolSeriesName = data.ToolSeries;
    ...
    let [customParams, toolData] = await prisma.$transaction([
    ...
    return { ...customParams, ...toolData, ...computedToolParameters };
}
...
app.get('/test', async (req, res) => {
    execFile("C:\\Users\\tedva\\Documents\\1_MA-Ford\\V2\\SW_Console_Controller_V1\\SW_Console_Controller_V1\\bin\\Debug\\SW_Console_Controller_V1.exe", (err, data) => {
        console.log(data);
    })
